    ...
    DO indxGrid=1,NParamGrids
      ...
      DO indxFENode=1,SIZE(NodesInterp)
        ...
        IF (Stat .EQ. .FALSE.) THEN
            CALL SetLastMessage('FE node/element ' // TRIM(IntTotext(NodesInterp(indxFENode))) // ' cannot be located in parametric grid number ' // TRIM(IntToText(indxGrid)) // ' for ' // TRIM(cDescription) // '!',iFatal,Thisprocedure)
            iStat = -1
            RETURN
        END IF
      END DO
        ...
        DO
            ...
            IF (IPOS_DASH .EQ. 0) THEN
                ...
                IF (ID .NE. 0) THEN
                    ...
                    IF (iLoc .EQ. 0) THEN
                        CALL SetLastMessage(cNodeOrElem // ' ID number ' // TRIM(IntToText(ID)) // ', listed in parametric grid number '// TRIM(IntToText(iParamGridNo)) // ' for ' // TRIM(cDescriptor) // ', is not in the model!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
