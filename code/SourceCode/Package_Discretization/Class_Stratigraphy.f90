...
! ******************************************************************
  ...
  SUBROUTINE ReadStratigraphyData(Stratigraphy,NNodes,NodeIDs,FileName,iStat) 
    ...
    DO indxNode=1,NNodes
        ...
        IF (index .EQ. 0) THEN
            CALL SetLastMessage('Node ID '//TRIM(IntToText(ID))//' listed for stratigraphy data is not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (lProcessed(index)) THEN
            CALL SetLastMessage('Node ID '//TRIM(IntToText(ID))//' is listed more than once for stratigraphy data!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxNode=1,NNodes
        IF (.NOT. lProcessed(indxNode)) THEN
            CALL SetLastMessage('Stratigraphy is not defined at node '//TRIM(IntToText(NodeIDs(indxNode)))//'!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    END DO
