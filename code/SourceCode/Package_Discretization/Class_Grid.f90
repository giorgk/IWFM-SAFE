...
! ******************************************************************
  ...
  SUBROUTINE New(Grid,X,Y,NVertex,Vertex,iStat)
    ...
    IF (ErrorCode .NE. 0) THEN
        CALL SetLastMessage('Error in allocating memory for nodal coordinates of a grid!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
  ...
  SUBROUTINE NodeArea_ForArrayList(Grid,NodeList,iNodeIDs,Area,iStat)
    ...
    DO indx=1,SIZE(NodeList)
        ...
        IF (Area(indx) .LE. 0.0) THEN
            ...
            CALL SetLastMessage('Nodal area at node '//TRIM(IntToText(ID))//' is zero!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    END DO
