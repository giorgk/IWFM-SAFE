...
! ******************************************************************
  ...
  SUBROUTINE ReadAppGridData(AppGrid,NodeFileName,ElementConfigFileName,iStat) 
    ...
    INTEGER                                   :: ErrorCode
    INTEGER,ALLOCATABLE                       :: Subregion_IDs(:),NVertex(:),Vertex_IDs(:,:),NodeID(:),ElemID(:)
    ...
    CALL ReadElementConfigData(ElementConfigFileName,ElemID,NVertex,Vertex_IDs,Subregion_IDs,cSubregionNames,iStat)  ;  IF (iStat .EQ. -1) RETURN
    
    !Instantiate application grid
    CALL ConstructAppGrid(X,Y,NVertex,Vertex_IDs,NodeID,ElemID,Subregion_IDs,cSubregionNames,AppGrid,iStat)  
    IF (iStat .EQ. -1) RETURN
    
    !Free memory
    DEALLOCATE (X , Y , NVertex , Vertex_IDs ,  Subregion_IDs , cSubregionNames , STAT=ErrorCode)
  ...
  SUBROUTINE ReadNodeData(FileName,ID,X,Y,iStat)
    ...
    DO indx=1,NNodes
        ...
        DO indx1=1,indx-1
            ...
            IF (ID1 .EQ. ID2) THEN
                 CALL SetLastMessage('Node ID ' // TRIM(IntToText(ID1)) // ' is defined more than once!',iFatal,ThisProcedure)
                 iStat = -1
                 RETURN
            END IF
            ...
            IF (X(indx1) .EQ. X(indx)) THEN
                IF (Y(indx1) .EQ. Y(indx)) THEN
                    CALL SetLastMessage('Nodes '//TRIM(IntToText(ID1))//' and '//TRIM(IntToText(ID2))//' have the same coordinates!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
            END IF
  ...
  ! -------------------------------------------------------------
  SUBROUTINE ReadElementConfigData(FileName,ID,NVertex,Vertex_ID,SubregionID,cSubregionNames,iStat) 
    CHARACTER(LEN=*),INTENT(IN)                           :: FileName
    INTEGER,ALLOCATABLE,INTENT(OUT)                       :: ID(:),NVertex(:),Vertex_ID(:,:),SubregionID(:)
    ...
    INTEGER                      :: indx,indx1,NElements,ErrorCode,NRegions,ID1,ID2
    ...
    ALLOCATE (ID(NElements) , NVertex(NElements) , Vertex_ID(4,NElements) , SubregionID(NElements) , cSubregionNames(NRegions) , STAT=ErrorCode)
    IF (ErrorCode.NE.0) THEN
    ...
    DO indx=1,NElements
        ...
        DO indx1=2,5
            IF (ANY(TDummy2DIntArray(indx1,indx) .EQ. TDummy2DIntArray(indx1+1:5,indx))) THEN
                CALL SetLastMessage('Repeating node numbers at element '//TRIM(IntToText(ID1))//'!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
        ...
        DO indx1=1,indx-1
            ID2 = ID(indx1)
            IF (ID1 .EQ. ID2) THEN
                 CALL SetLastMessage('Element ID ' // TRIM(IntToText(ID1)) // ' is defined more than once!',iFatal,ThisProcedure)
                 iStat = -1
                 RETURN                
            END IF
        END DO
    ...
    CALL GetUniqueArrayComponents(SubregionID,UniqueSubregionID)
    IF (SIZE(UniqueSubregionID) .NE. NRegions) THEN
        MessageArray(1) = 'Number of subregions defined in NREGN is different than'
        MessageArray(2) = 'the subregion IDs specified for elements!'
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE ConstructAppGrid(X,Y,NVertex,Vertex_IDs,NodeID,ElemID,Subregion_IDs,cSubregionNames,AppGrid,iStat)
    REAL(8),INTENT(IN)            :: X(:),Y(:)
    INTEGER,INTENT(IN)            :: NVertex(:),Vertex_IDs(:,:),NodeID(:),ElemID(:),Subregion_IDs(:)
    ...
    INTEGER                       :: indx,ConvexNode,indxVertex,ErrorCode,NElements,NNodes,iCount,indxFace,iStatParallel,Vertexc(4),    &
                                     ...
                                     Subregion_Indices(SIZE(Subregion_IDs))
    REAL(8)                       :: Xc(4),Yc(4)
    INTEGER,ALLOCATABLE           :: DummyIntArray(:),SubregionIDs_Unique(:)
    ...
    CALL GetUniqueArrayComponents(Subregion_IDs,SubregionIDs_Unique)
    CALL ShellSort(SubregionIDs_Unique)
    CALL ConvertID_To_Index(Subregion_IDs,SubregionIDs_Unique,Subregion_Indices)
    ...
    AppGrid%AppElement%Subregion = Subregion_Indices
    ...
    DO indx=1,NElements
        ...
        IF (ErrorCode .NE. 0) THEN
            CALL SetLastMessage('Error in allocating memory for vertex areas for element '//TRIM(IntToText(AppGrid%AppElement(indx)%ID))//'!',iFatal,ThisProcedure)          
            iStatParallel = -1
        END IF
    ...
    DO indx=1,NNodes
        ...
        IF (iStat .EQ. -1) THEN
            iStatParallel = -1
        END IF
        IF (SIZE(AppGrid%AppNode(indx)%SurroundingElement) .EQ. 0) CALL LogMessage('Node number '//TRIM(IntToText(AppGrid%AppNode(indx)%ID))//' does not have any surrounding elements!',iWarn,ThisProcedure)
    END DO
    !$OMP END DO 
    !$OMP END PARALLEL  
    IF (iStatParallel .EQ. -1) THEN
    ...
    DO indx=1,NNodes
        ...
        IF (AppGrid%AppNode(indx)%NConnectedNode .EQ. 0) CALL LogMessage('Node number '//TRIM(IntToText(AppGrid%AppNode(indx)%ID))//' does not have any connected nodes!',iWarn,ThisProcedure)
    END DO
    !$OMP END DO 
    !$OMP END PARALLEL  
    IF (iStatParallel .EQ. -1) THEN
    ...
    DO indx=1,NNodes
        ...
        IF (AppGrid%AppNode(indx)%NFaceID .EQ. 0) CALL LogMessage('Node number '//TRIM(IntToText(AppGrid%AppNode(indx)%ID))//' does not have any faces connecting at it!',iWarn,ThisProcedure)
    END DO
    !$OMP END DO 
    !$OMP END PARALLEL  
    IF (iStatParallel .EQ. -1) THEN
    ...
    DO indx=1,NNodes
        ...
        IF (iStat .EQ. -1) THEN
            iStatParallel = -1
        END IF
        IF (SIZE(AppGrid%AppNode(indx)%ElemID_OnCCWSide) .EQ. 0) CALL LogMessage('Node number '//TRIM(IntToText(AppGrid%AppNode(indx)%ID))//' does not have any surrounding elements!',iWarn,ThisProcedure)
    END DO
    !$OMP END DO 
    !$OMP END PARALLEL  
    IF (iStatParallel .EQ. -1) THEN
    ...
    CALL CompileSubregionalData(AppGrid,SubregionIDs_Unique,cSubregionNames,iStat)  
    IF (iStat .EQ. -1) RETURN
        ...
        DO indxNode=1,iNNodesInError
            ...
            IF (MOD(iCount,15) .EQ. 0) THEN
                IF (indxNode .LT. iNNodesInError) THEN
                    iCount          = 0
                    MessageArray(2) = TRIM(MessageArray(2)) // ',' // LineFeed // '*   '
                END IF
            END IF
        END DO
        CALL SetLastMessage(MessageArray(1:2),iFatal,ThisProcedure)
  ...
  CONTAINS
    ...
    SUBROUTINE PrintInternalBndNodes(iDim,iBndNodes,iNodeIDs)
      ...
      END DO
      CALL LogMessage(MessageArray(1:MIN(indxMsgArray,iMaxDim)),iWarn,ThisProcedure)
