...
100 CALL ZoneDefFile%Kill()
  ...
  SUBROUTINE GenerateZoneList(ZoneList,iNDataCols,lFaceFlows_Defined,SystemData,iZExtent,iElems,iLayers,iZones,iZonesWithNames,cZoneNames,iStat)
    ...
    IF (ANY(iElemIndices.EQ.0)) THEN
        CALL SetLastMessage('One or more element numbers defined for zonation are not in the model!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
  ...
  SUBROUTINE ReadElemZoneInformation(InputFile,NElements,NLayers,ZExtent,ElementIDs,ElemZones,iStat)
    ...
    DO indx=1,SIZE(DummyCharArray)
        ...
        IF (ElementIndex .EQ. 0) THEN
            CALL SetLastMessage('Element number '//TRIM(IntToText(ElementID))//' is not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (ZExtent .EQ. f_iZoneHorizontal) THEN
            ...
            IF (ANY(ElemZones(ElementIndex,:) .NE. f_iUndefinedZone)) THEN
                CALL SetLastMessage('Multiple zone numbers are specified for element '//TRIM(IntToText(ElementID))//'!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        ...
        ELSE
            ...
            IF (ElemZones(ElementIndex,LayerNo) .NE. f_iUndefinedZone) THEN
                CALL SetLastMessage('Multiple zone numbers are specified for element '//TRIM(IntToText(ElementID))//' in layer '//TRIM(IntToText(LayerNo))//'!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
