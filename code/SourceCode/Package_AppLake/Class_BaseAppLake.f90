...
! ******************************************************************
  ...
  SUBROUTINE GetDataList_AtLocationType(AppLake,iLocationType,cDataList,cFileList,lBudgetType) 
    ...
    IF (iLocationType .NE. f_iLocationType_Lake) RETURN
  ...
  SUBROUTINE GetLocationsWithData(AppLake,iLocationType,cDataType,iLocations) 
    ...
    IF (iLocationType .NE. f_iLocationType_Lake) THEN
  ...
  ! -------------------------------------------------------------
  SUBROUTINE GetSubDataList_ForLocationAndDataType(AppLake,iLocationType,cDataType,cSubDataList) 
    CLASS(BaseAppLakeType),INTENT(IN)        :: AppLake
    INTEGER,INTENT(IN)                       :: iLocationType
    ...
    IF (iLocationType .EQ. f_iLocationType_Lake) THEN
  ...
  SUBROUTINE GetModelData_AtLocation(AppLake,iLocationType,iLocationID,cDataType,iCol,cOutputBeginDateAndTime,cOutputEndDateAndTime,cOutputInterval,rFact_LT,rFact_AR,rFact_VL,iDataUnitType,nActualOutput,rOutputDates,rOutputValues,iStat)
    ...
    IF (.NOT. (iLocationType.EQ.f_iLocationType_Lake  .AND.  TRIM(cDataType).EQ.cDataList_AtLake)) RETURN
    
    !If lake budget output is not defined return
    IF (.NOT. AppLake%LakeBudRawFile_Defined) RETURN
    
    !Convert lake ID to index
    iLake = LocateInList(iLocationID,AppLake%Lakes%ID)
    IF (iLake .EQ. 0) THEN
        CALL SetLastMessage('Lake ID '//TRIM(IntToText(iLocationID))//' listed for results retrieval is not in the model!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    
    !Get the data for the specified budget column for the specified period with specified interval
    CALL AppLake%LakeBudRawFile%ReadData(iLake,iCol,cOutputInterval,cOutputBeginDateAndTime,cOutputEndDateAndTime,1d0,0d0,0d0,rFact_LT,rFact_AR,rFact_VL,iDataUnitType,nActualOutput,rOutputDates,rOutputValues,iStat)
    ...
    DO indxLake=1,NLakes
        ASSOCIATE (pLake => AppLake%Lakes(indxLake))
            ...
            ALLOCATE (pLake%Elements(pLake%NElements) , &
                      ...
                      STAT=ErrorCode                  )
            IF (ErrorCode .NE. 0) THEN
                CALL SetLastMessage('Error in allocating memory for lake '//TRIM(IntToText(pLake%ID))//'!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
...
! ******************************************************************
    ...
    DO indx=1,AppLake%NLakes
        IF (AppLake%Lakes(indx)%OutflowDestType .EQ. f_iFlowDest_StrmNode) THEN
            ...
            IF (iDest .EQ. 0) THEN
                ...
                CALL SetLastMessage('Stream node '//TRIM(IntToText(iDestID))//' that receive outflow from lake '//TRIM(IntToText(iLakeID))//' is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
  ...
  FUNCTION PrepareLakeBudgetHeader(Lakes,NTIME,TimeStep,cVersion) RESULT(Header)
    ...
    ASSOCIATE (pLocation => Header%Locations(1))
      ...
      pLocation%cFullColumnHeaders(NLakeBudColumns+1) = TRIM(pLocation%cFullColumnHeaders(NLakeBudColumns+1)) // ' ('//f_cLengthUnitMarker//')'
    ...
    ASSOCIATE (pDSSOutput => Header%DSSOutput)
      ...
      DO indxLocation=1,NLakes
        ...
        END DO
      END DO
      pDSSOutput%iDataTypes = [(f_iPER_CUM,indxCol=1,NLakeBudColumns-1),f_iPER_AVER]
    END ASSOCIATE
