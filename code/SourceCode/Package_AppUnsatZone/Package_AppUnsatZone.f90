...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE GetSubDataList_ForLocationAndDataType(AppUnsatZone,iLocationType,cDataType,cSubDataList)
    CLASS(AppUnsatZoneType),INTENT(IN)       :: AppUnsatZone
    INTEGER,INTENT(IN)                       :: iLocationType
    CHARACTER(LEN=*),INTENT(IN)              :: cDataType
  ...
  SUBROUTINE GetModelData_AtLocation(AppUnsatZone,iSubregionIDs,iZExtent,iElems,iLayers,iZones,iZonesWithNames,cZoneNames,iLocationType,iLocationID,cDataType,iCol,cOutputBeginDateAndTime,cOutputEndDateAndTime,cOutputInterval,rFact_LT,rFact_AR,rFact_VL,iDataUnitType,nActualOutput,rOutputDates,rOutputValues,iStat)
    ...
    INTEGER                                :: iReadCols(1),iDataUnitTypeArray(1),iLocationIndex
    ...
    SELECT CASE (iLocationType)
        CASE (f_iLocationType_Subregion)
            IF (TRIM(cDataType) .EQ. cDataList_AtSubregion) THEN
                IF (ALLOCATED(AppUnsatZone%BudRawFile)) THEN
                    CALL ConvertID_To_Index(iLocationID,iSubregionIDs,iLocationIndex)
                    IF (iLocationIndex .EQ. 0) THEN
                        ...
                        RETURN
                    END IF                    
                    CALL AppUnsatZone%BudRawFile%ReadData(iLocationIndex,iCol,cOutputInterval,cOutputBeginDateAndTime,cOutputEndDateAndTime,1d0,0d0,0d0,rFact_LT,rFact_AR,rFact_VL,iDataUnitType,nActualOutput,rOutputDates,rOutputValues,iStat)
                END IF
            END IF
    ...
    IF (.NOT. (iLocationType.EQ.f_iLocationType_Subregion  .AND.  TRIM(cDataType).EQ.cDataList_AtSubregion)) RETURN
  ...
  SUBROUTINE ReadInitialConditions(InFile,iElemIDs,NUZLayers,UnsatElems,iStat) 
    ...
    ELSE
        ...
        DO indxElem=1,SIZE(UnsatElems,DIM=2)
            ...
            IF (iElem .EQ. 0) THEN
                CALL SetLastMessage('Element '//TRIM(IntToText(ID))//' listed for unsaturated zone initial conditions is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            ...
            IF (lProcessed(iElem)) THEN
                CALL SetLastMessage('Element '//TRIM(IntToText(ID))//' specified for unsaturated zone initail conditions is listed more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
  ...
  SUBROUTINE ReadUnsatZoneParameters(NUZLayers,AppGrid,Stratigraphy,TimeStep,InFile,VarTimeUnit,UnsatElems,iStat)
    ...
    IF (NGroup .EQ. 0) THEN
        ...
        DO indxElem=1,NElements
            ...
            CALL ConvertID_To_Index(ID,iElemIDs,iElem)
            IF (iElem .EQ. 0) THEN 
                CALL SetLastMessage('Element '//TRIM(IntToText(ID))//' listed for unsaturated zone parameters is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            ...
            IF (lProcessed(iElem)) THEN
                CALL SetLastMessage('Element '//TRIM(IntToText(ID))//' specified for unsatuarted zone parameters is listed more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
