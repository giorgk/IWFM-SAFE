...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  PURE FUNCTION GetNDataList_AtLocationType(AppSWShed,iLocationType) RESULT(NData)
    CLASS(AppSmallWatershedType),INTENT(IN) :: AppSWShed
    INTEGER,INTENT(IN)                      :: iLocationType
    INTEGER                                 :: NData
    
    !If defined location type is not small watersheds return
    IF (iLocationType .NE. f_iLocationType_SmallWatershed) THEN
        NData = 0
        RETURN
    END IF
  ...
  SUBROUTINE GetDataList_AtLocationType(AppSWShed,iLocationType,cDataList,cFileList,lBudgetType)
    ...
    IF (iLocationType .NE. f_iLocationType_SmallWatershed) RETURN
  ...
  ! -------------------------------------------------------------
  SUBROUTINE GetSubDataList_ForLocationAndDataType(AppSWShed,iLocationType,cDataType,cSubDataList)
    CLASS(AppSmallWatershedType),INTENT(IN)  :: AppSWShed
    INTEGER,INTENT(IN)                       :: iLocationType
    ...
    IF (iLocationType .EQ. f_iLocationType_SmallWatershed) THEN
  ...
  SUBROUTINE GetModelData_AtLocation(AppSWShed,iLocationType,iLocationID,cDataType,iCol,cOutputBeginDateAndTime,cOutputEndDateAndTime,cOutputInterval,rFact_LT,rFact_AR,rFact_VL,iDataUnitType,nActualOutput,rOutputDates,rOutputValues,iStat)
    ...
    IF (.NOT. (iLocationType.EQ.f_iLocationType_SmallWatershed  .AND.  TRIM(cDataType).EQ.cDataList_AtSmallWatershed)) RETURN
    
    !If small watershed budget output is not defined return
    IF (.NOT. AppSWShed%lBudRawFile_Defined) RETURN
    
    !Convert small watershed ID to index
    iSWShed = LocateInList(iLocationID,AppSWShed%SmallWatersheds%ID)
    IF (iSWShed .EQ. 0) THEN
        CALL SetLastMessage('Small watershed ID '//TRIM(IntToText(iLocationID))//' listed for results retrieval is not in the model!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    
    !Get the data for the specified budget column for the specified period with specified interval
    CALL AppSWShed%BudRawFile%ReadData(iSWShed,iCol,cOutputInterval,cOutputBeginDateAndTime,cOutputEndDateAndTime,1d0,0d0,0d0,rFact_LT,rFact_AR,rFact_VL,iDataUnitType,nActualOutput,rOutputDates,rOutputValues,iStat)
        ...
        DO indxSWShed=1,AppSWShed%NSWShed
            ...
            DO indx=1,indxSWShed-1
                IF (iSWShedID .EQ. pSWSheds(indx)%ID) THEN
                    CALL SetLastMessage('Small watershed ID number '//TRIM(IntToText(iSWShedID))//' is used more than once!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
            END DO
            ...
            IF (ANY(iGWNodes.EQ.0)) THEN
                CALL SetLastMessage('One or more groundwater nodes listed for small watershed '//TRIM(IntToText(iSWShedID))//' are not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
  ...
  SUBROUTINE ReadRootZoneData(InFile,TimeStep,iSWShedIDs,AppSWShed,iStat)
    ...
    ASSOCIATE (pSWSheds => AppSWShed%SmallWatersheds)
        DO indxSWShed=1,AppSWShed%NSWShed
            ...
            IF (iSWShed .EQ. 0) THEN
                CALL SetLastMessage('Small watershed ID '//TRIM(IntToText(iSWShedID))//' listed for root zone parameter definition is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            IF (lProcessed(iSWShed)) THEN
                CALL SetLastMessage('Small watershed ID '//TRIM(IntToText(iSWShedID))//' is listed more than once for root zone parameter definitions!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            ...
            IF (pSWSheds(iSWShed)%Soil%TotalPorosity .GT. 1.0) THEN
                CALL SetLastMessage('At small watershed ' // TRIM(IntToText(iSWShedID)) // ' total porosity is greater than 1.0!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            ...
            IF (pSWSheds(iSWShed)%Soil%WiltingPoint .GE. pSWSheds(iSWShed)%Soil%FieldCapacity) THEN
                CALL SetLastMessage('At small watershed ' // TRIM(IntToText(iSWShedID)) // ' wilting point is greater than or equal to field capacity!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            ...
            IF (pSWSheds(iSWShed)%Soil%FieldCapacity .GT. pSWSheds(iSWShed)%Soil%TotalPorosity) THEN
                CALL SetLastMessage('At small watershed ' // TRIM(IntToText(iSWShedID)) // ' field capacity is greater than total porosity!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            ...
            IF (.NOT. ANY(pSWSheds(iSWShed)%Soil%KunsatMethod .EQ. f_iKunsatMethodList)) THEN
                CALL SetLastMessage('Method to compute unsaturated hydraulic conductivity at small watershed '//TRIM(IntToText(iSWShedID))//' is not recognized!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO       
  ...
  SUBROUTINE ReadAquiferData(InFile,TimeStep,iSWShedIDs,AppSWShed,iStat)
    ...
    ASSOCIATE (pSWSheds => AppSWShed%SmallWatersheds)
        DO indxSWShed=1,AppSWShed%NSWShed
            ...
            CALL ConvertID_To_Index(iSWShedID,iSWShedIDs,iSWShed)
            IF (iSWShed .EQ. 0) THEN
                CALL SetLastMessage('Small watershed ID '//TRIM(IntToText(iSWShedID))//' listed for aquifer parameter definition is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            IF (lProcessed(iSWShed)) THEN
                CALL SetLastMessage('Small watershed '//TRIM(IntToText(iSWShedID))//' is listed more than once for aquifer parameter definitions!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iSWShed) = .TRUE.
  ...
  SUBROUTINE ReadInitialConditions(InFile,iSWShedIDs,AppSWShed,iStat)
    ...
    ASSOCIATE (pSWSheds => AppSWShed%SmallWatersheds)
        DO indxSWShed=1,AppSWShed%NSWShed           
            ...
            CALL ConvertID_To_Index(iSWShedID,iSWSHedIDs,iSWShed)
            IF (iSWShed .EQ. 0) THEN
                CALL SetLastMessage('Small watershed ID '//TRIM(IntToText(iSWShedID))//' listed for initial conditions is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            IF (lProcessed(iSWShed)) THEN
                CALL SetLastMessage('Small watershed ID '//TRIM(IntToText(iSWShedID))//' is listed more than once for initial conditions definition!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iSWShed) = .TRUE.
            ...
            IF (DummyArray(2) .GT. 1.0   .OR.  DummyArray(2) .LT. 0.0) THEN
                CALL SetLastMessage('The initail soil moisture content at small watershed ID '//TRIM(IntToText(iSWShedID))// ' must be between 0.0 and 1.0!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
