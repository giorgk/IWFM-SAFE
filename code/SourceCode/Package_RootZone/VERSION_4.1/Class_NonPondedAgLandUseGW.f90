...
MODULE Class_NonPondedAgLandUseGW
  ...
  USE Util_RootZone_v41             , ONLY: WaterSupplyType                  , &
  ...
  SUBROUTINE New(AgLand,IsForInquiry,cFileName,cWorkingDirectory,FactCN,AppGrid,iElemIDs,TimeStep,NTimeSteps,cVersion,iStat)
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL SetLastMessage('Curve numbers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL SetLastMessage('Evapotranspration column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
        END IF
        lProcessed(iElem)             = .TRUE.
    ...
    IF (.NOT. ALL(DummyIntArray(:,2:).EQ.0)) THEN
        ALLOCATE (AgLand%iColAgDemand(NCrops,NElements) ,STAT=ErrorCode)
        IF (ErrorCode .NE. 0) THEN
            CALL SetLastMessage('Error in allocating memory for non-ponded agricultural water supply requirement column pointers!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed = .FALSE.
        DO indxElem=1,NElements
            ...
            IF (lProcessed(iElem)) THEN
                ID = iElemIDs(iElem)
                CALL SetLastMessage('Water supply requirement column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iElem)            = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL SetLastMessage('Irrigation period column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                    = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL SetLastMessage('Minimum soil moisture column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                  = .TRUE.
    ...
    IF (ALine .NE. '') THEN
        ...
        DO indxElem=1,NElements
            ...
            IF (lProcessed(iElem)) THEN
                ID = iElemIDs(iElem)
                CALL SetLastMessage('Irrigation target soil moisture column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iElem)                     = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL SetLastMessage('Return flow fraction column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                    = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL SetLastMessage('Re-use flow fraction column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                   = .TRUE.
    ...
    IF (ALine .NE. '') THEN
        ...
        DO indxElem=1,NElements
            ...
            IF (lProcessed(iElem)) THEN
                ID = iElemIDs(iElem)
                CALL SetLastMessage('Minimum percolation column pointers for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iElem)                   = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL SetLastMessage('Initial conditions for non-ponded crops at element '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                  = .TRUE.
      ...
      ASSOCIATE (pCrops => NonPondedAgLand%Crops(:,indxElem)) 
        ...
        DO indxCrop=1,NCrops
            IF (pCrops(indxCrop)%SoilM_Precip + pCrops(indxCrop)%SoilM_AW + pCrops(indxCrop)%SoilM_Oth .GT. TP) THEN
                CALL SetLastMessage('Initial moisture content for crop ' // TRIM(NonPondedAgLand%CropCodes(indxCrop)) // ' at element ' // TRIM(IntToText(iElemIDs(indxElem))) // ' is greater than total porosity!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
