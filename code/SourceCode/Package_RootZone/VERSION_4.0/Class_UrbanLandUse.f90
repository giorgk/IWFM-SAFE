...
MODULE Class_UrbanLandUse
  ...
  USE Util_RootZone_v40       , ONLY: ReadRealData                   
  IMPLICIT NONE
...
! ******************************************************************
  ...
  SUBROUTINE New(UrbLand,cFileName,cWorkingDirectory,FactCN,NElements,NSubregions,iElemIDs,TrackTime,iStat)
    ...
    ASSOCIATE (pUrbData  => UrbLand%UrbData  )
        ...
        DO indxElem=1,NElements
            ...
            IF (lProcessed(iElem)) THEN
                ID = iElemIDs(iElem)
                CALL SetLastMessage('Data for urban lands at element '//TRIM(IntToText(ID))//' is defined more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed(iElem)) THEN
            ID = iElemIDs(iElem)
            CALL SetLastMessage('initial conditions for urban lands at element '//TRIM(IntToText(ID))//' is defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iElem)                                  = .TRUE.
    ...
    DO indxElem=1,NElements
      ASSOCIATE (pUrbData => UrbanLand%UrbData(indxElem)) 
        ...
        IF ((pUrbData%SoilM_Precip + pUrbData%SoilM_AW + pUrbData%SoilM_Oth) .GT. TotalPorosity(indxElem)) THEN
            CALL SetLastMessage('Initial moisture content for urban land at element ' // TRIM(IntToText(iElemIDs(indxElem))) // ' is greater than total porosity!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
