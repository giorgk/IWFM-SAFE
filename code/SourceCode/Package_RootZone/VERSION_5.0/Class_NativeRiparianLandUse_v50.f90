    ...
    ASSOCIATE (pNativeVeg   => NVRVLand%NativeVeg   , &
        ...
        DO indxRegion=1,NSubregions
            ...
            IF (lProcessed(iRegion)) THEN
                ...
                CALL SetLastMessage('curve numbers for native and riparian vegetation at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        ...
        DO indxRegion=1,NSubregions
            ...
            IF (lProcessed(iRegion)) THEN
                ID = iSubregionIDs(iRegion)
                CALL SetLastMessage('evapotranspiration column pointers for native and riparian vegetation at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iRegion)             = .TRUE.
        ...
        DO indxRegion=1,NSubregions 
            ...
            IF (lProcessed(iRegion)) THEN
                ID = iSubregionIDs(iRegion)
                CALL SetLastMessage('initial conditions for native and riparian vegetation at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iRegion)                  = .TRUE.
        ...
        DO indxRegion=1,NRegions
            DO indxSoil=1,NSoils
                IF ((pNV(indxSoil,indxRegion)%SoilM_Precip + pNV(indxSoil,indxRegion)%SoilM_AW + pNV(indxSoil,indxRegion)%SoilM_Oth) .GT. TotalPorosity(indxSoil,indxRegion)) THEN
                    CALL SetLastMessage('Initial moisture content for native vegetation with soil type ' // TRIM(IntToText(indxSoil)) // ' at subregion ' // TRIM(IntToText(iSubregionIDs(indxRegion))) // ' is greater than total porosity!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
                IF ((pRV(indxSoil,indxRegion)%SoilM_Precip + pRV(indxSoil,indxRegion)%SoilM_AW + pRV(indxSoil,indxRegion)%SoilM_Oth) .GT. TotalPorosity(indxSoil,indxRegion)) THEN
                    CALL SetLastMessage('Initial moisture content for riparian vegetation with soil type ' // TRIM(IntToText(indxSoil)) // ' at subregion ' // TRIM(IntToText(iSubregionIDs(indxRegion))) // ' is greater than total porosity!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
