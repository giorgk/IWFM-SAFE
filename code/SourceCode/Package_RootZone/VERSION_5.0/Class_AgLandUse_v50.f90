  ...
  SUBROUTINE New(AgLand,IsForInquiry,cFileName,cWorkingDirectory,AppGrid,FactCN,NSoils,iSubregionIDs,TimeStep,iStat)
    ...
    DO indxRegion=1,NSubregions
        ...
        IF (lProcessed(iRegion)) THEN
            ...
            CALL SetLastMessage('Curve numbers for agricultural crops at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxRegion=1,NSubregions
        ...
        IF (lProcessed(iRegion)) THEN
            ID = iSubregionIDs(iRegion)
            CALL SetLastMessage('Evapotranspration column pointers for agricultural crops at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iRegion)           = .TRUE.
    ...
    DO indxRegion=1,NSubregions
        ...
        IF (lProcessed(iRegion)) THEN
            ID = iSubregionIDs(iRegion)
            CALL SetLastMessage('Irrigation period column pointers for agricultural crops at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iRegion)              = .TRUE.
    ...
    IF (ALine .NE. '') THEN
        ...
        DO indxRegion=1,NSubregions
            ...
            IF (lProcessed(iRegion)) THEN
                ID = iSubregionIDs(iRegion)
                CALL SetLastMessage('Irrigation target soil moisture pointers for agricultural crops at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            lProcessed(iRegion)               = .TRUE.
    ...
    DO indxRegion=1,NSubregions
        ...
        IF (lProcessed(iRegion)) THEN
            ID = iSubregionIDs(iRegion)
            CALL SetLastMessage('Water supply requirements, return flow and re-use fractions for agricultural crops at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iRegion)            = .TRUE.
    ...
    DO indxRegion=1,NSubregions
        ...
        IF (lProcessed(iRegion)) THEN
            ID = iSubregionIDs(iRegion)
            CALL SetLastMessage('Initial conditions for agricultural crops at subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iRegion)                                  = .TRUE.
    ...
    IF (AgLand%IrigPeriodFile%lUpdated) THEN
        DO indxCol=1,AgLand%IrigPeriodFile%iSize
            IF (LocateInList(AgLand%IrigPeriodFile%iValues(indxCol) , f_iIrigPeriodFlags) .EQ. 0) THEN
                CALL SetLastMessage('One or more irrigation period flags are not recognized!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
            ...
            DO indxRegion=1,AppGrid%NSubregions
                ...
                IF (AgLand%SubregionalDemand(indxRegion) .GT. 0.0) THEN
                    IF (AgLand%SubregionalArea(indxRegion) .EQ. 0.0) THEN
                        CALL SetLastMessage('Agricultural water supply requirement at subregion '//TRIM(IntToText(iSubregionIDs(indxRegion)))//' is greater than zero when agricultural area is zero!',iFatal,ThisProcedure) 
                        iStat = -1
                        RETURN
                    END IF
                END IF
        ...
        DO indxRegion=1,AppGrid%NSubregions
           ...
           IF (SUM(AgLand%SubregionalCropAreaFrac(:,indxRegion)) .EQ. 0.0) THEN
               IF (AgLand%SubregionalArea(indxRegion) .GT. 0.0) THEN
                   CALL SetLastMessage('Subregional crop areas cannot be all zero in subregion '//TRIM(IntToText(iSubregionIDs(indxRegion)))//' when elemental agricultural areas are non-zero!',iFatal,ThisProcedure)
                   iStat = -1
                   RETURN
               END IF
           END IF 
        ...
        DO indxRegion=1,NRegions
            DO indxSoil=1,NSoils
                IF ((pAgData(indxSoil,indxRegion)%SoilM_Precip + pAgData(indxSoil,indxRegion)%SoilM_AW + pAgData(indxSoil,indxRegion)%SoilM_Oth) .GT. TotalPorosity(indxSoil,indxRegion)) THEN
                    CALL SetLastMessage('Initial moisture content for agricultural lands with soil type ' // TRIM(IntToText(indxSoil)) // ' at subregion ' // TRIM(IntToText(iSubregionIDs(indxRegion))) // ' is greater than total porosity!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
