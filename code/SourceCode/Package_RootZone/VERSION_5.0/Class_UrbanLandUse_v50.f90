  ...
  SUBROUTINE New(UrbLand,cFileName,cWorkingDirectory,AppGrid,FactCN,NSoils,iElemIDs,iSubregionIDs,TrackTime,iStat,iStrmNodeIDs,iLakeIDs)
    ...
    DO indxRegion=1,NSubregions
        ...
        IF (lProcessed(iRegion)) THEN
            ...
            CALL SetLastMessage('Curve numbers for urban lands for subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxRegion=1,NSubregions
        ...
        IF (lProcessed(iRegion)) THEN
            ID = iSubregionIDs(iRegion)
            CALL SetLastMessage('Water use and management data for urban lands for subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iRegion)                = .TRUE.
    ...
    DO indxElem=1,NElements
        ...
        IF (lProcessed_Elem(iElem)) THEN
            CALL SetLastMessage('Surface flow destination for urban lands for element '//TRIM(IntToText(ID))//' is defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (SurfaceFlowDestType(iElem) .NE. f_iFlowDest_Outside    .AND.   &
            ...
            CALL SetLastMessage('Surface flow destination type for urban surface runoff at element ' // TRIM(IntToText(ID)) // ' is not recognized!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        SELECT CASE (SurfaceFlowDestType(iElem))               
            CASE (f_iFlowDest_StrmNode)
                IF (PRESENT(iStrmNodeIDs)) THEN
                    ...
                    IF (iStrmNode .EQ. 0) THEN
                        CALL SetLastMessage('Urban surface flow from element '//TRIM(IntToText(ID))//' flows into a stream node ('//TRIM(IntToText(SurfaceFlowDest(iElem)))//') that is not modeled!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
            ...
            CASE (f_iFlowDest_Lake)
                IF (PRESENT(iLakeIDs)) THEN
                    ...
                    IF (iStrmNode .EQ. 0) THEN
                        CALL SetLastMessage('Urban surface flow from element '//TRIM(IntToText(ID))//' flows into a lake ('//TRIM(IntToText(SurfaceFlowDest(iElem)))//') that is not modeled!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
            ...
            CASE (f_iFlowDest_Subregion)
                ...
                IF (iRegion .EQ. 0) THEN
                    CALL SetLastMessage('Urban surface flow from element '//TRIM(IntToText(ID))//' goes to a subregion ('//TRIM(IntToText(SurfaceFlowDest(iElem)))//') that is not modeled!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
                SurfaceFlowDest(iElem) = iRegion
                IF (SurfaceFlowDest(iElem) .EQ. AppGrid%AppElement(iElem)%Subregion) THEN
                    CALL SetLastMessage('Urban surface flow from element '//TRIM(IntToText(ID))//' cannot go to the same subregion which the element belongs to!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
        END SELECT              
    ...
    DO indxRegion=1,NSubregions
        ...
        IF (lProcessed(iRegion)) THEN
            ID = iSubregionIDs(iRegion)
            CALL SetLastMessage('Initial conditions for urban lands for subregion '//TRIM(IntToText(ID))//' are defined more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        lProcessed(iRegion)                                    = .TRUE.
        ...
        DO indxRegion=1,NRegions
            DO indxSoil=1,NSoils
                IF ((pUrbData(indxSoil,indxRegion)%SoilM_Precip + pUrbData(indxSoil,indxRegion)%SoilM_AW + pUrbData(indxSoil,indxRegion)%SoilM_Oth) .GT. TotalPorosity(indxSoil,indxRegion)) THEN
                    CALL SetLastMessage('Initial moisture content for urban land with soil type ' // TRIM(IntToText(indxSoil)) // ' at subregion ' // TRIM(IntToText(iSubregionIDs(indxRegion))) // ' is greater than total porosity!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
