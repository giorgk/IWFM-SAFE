  ...
  SUBROUTINE LandUseDataFile_ReadTSData(LandUseDataFile,cDescriptor,TimeStep,Area,iLocationIDs,iStat)
    ...
    DO indxLocation=1,NLocations
        ...
        IF (iLocation .EQ. 0) THEN
            CALL SetLastMessage('Element or subregion number '//TRIM(IntToText(iLocationID))//' listed for '//TRIM(LowerCase(cDescriptor))//' is not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (lProcessed(iLocation)) THEN
            CALL SetLastMessage('Element or subregion number '//TRIM(IntToText(iLocationID))//' listed for '//TRIM(LowerCase(cDescriptor))//' is listed more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
