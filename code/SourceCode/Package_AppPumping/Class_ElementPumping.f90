...
! ******************************************************************
  ...
  SUBROUTINE ElemPump_New(cFileName,AppGrid,Stratigraphy,ElemPump,iStat)
    ...
    DO indxSink=1,NSink
        ...
        IF (iElem .EQ. 0) THEN
            CALL SetLastMessage('Element '//TRIM(IntToText(iElemID))//' listed for element pumping is not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (ErrorCode .NE. 0) THEN
            CALL SetLastMessage('Error in allocating memory for the vertical distribution fractions for pumping at element ' // TRIM(IntToText(iElemID)) // '!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (.NOT. ANY(ElemPump(indxSink)%iDistMethod .EQ. f_iDistTypeArray)) THEN
            CALL SetLastMessage('Pumping distribution option (IOPTSK) for element ' // TRIM(IntToText(iElemID)) // ' is not recognized!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (.NOT. ANY(ElemPumpDest(indxSink)%iDestType .EQ. f_iDestTypeArray)) THEN
            CALL SetLastMessage('Destination type for element '//TRIM(IntToText(iElemID))//' is not recognized!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        ELSE
            SELECT CASE (ElemPumpDest(indxSink)%iDestType)
                ...
                CASE (f_iFlowDest_Element)
                    ...
                    IF (iDest .EQ. 0) THEN
                        CALL SetLastMessage('Destination element '//TRIM(IntToText(iDestID))//' listed for pumping at element '//TRIM(IntToText(iElemID))//' is not in the model!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
                ...
                CASE (f_iFlowDest_Subregion)
                    ...
                    IF (iDest .EQ. 0) THEN
                        CALL SetLastMessage('Subregion '//TRIM(IntToText(iDestID))//' listed as the destination for pumping from element '//TRIM(IntToText(iElemID))//' is not in the model!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
                    ElemPumpDest(indxSink)%iDest       = iDest
    ...
    DO indxGroup=1,iNGroup
        ...
        DO indxGroup1=1,indxGroup-1
            IF (ID .EQ. ElemGroups(indxGroup1)%ID) THEN
                CALL SetLastMessage('Element group ID '//TRIM(IntToText(ID))//' for element pumping destinations is specified more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
        ...
        IF (ANY(Indices.EQ.0)) THEN
            CALL SetLastMessage('One or more elements listed in element group ID '//TRIM(IntToText(ID))//' listed for element pumping destination are not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxSink=1,NSink
       IF (ElemPumpDest(indxSink)%iDestType .EQ. f_iFlowDest_ElementSet) THEN
           ...
           IF (iDest .EQ. 0) THEN
               ...
               CALL SetLastMessage('Element group number '//TRIM(IntToText(iDestID))//' to which pumping from element '//TRIM(IntToText(ID))//' is delivered is not defined!',iFatal,ThisProcedure)  
               iStat = -1
               RETURN
           END IF
           ElemPumpDest(indxSink)%iDest = iDest
           ...
           IF (ElemGroups(iDest)%NElems .EQ. 0) THEN
               ID = ElemPump(indxSink)%ID
               CALL SetLastMessage('Element group '//TRIM(IntToText(iDestID))//' as destination for pumping from element '//TRIM(IntToText(ID))//' has no elements listed!',iFatal,ThisProcedure)
               iStat = -1
               RETURN
           END IF
