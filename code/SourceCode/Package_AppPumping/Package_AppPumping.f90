...
! ******************************************************************
  ...
  SUBROUTINE New(AppPumping,cFileName,cWorkingDirectory,AppGrid,Stratigraphy,TimeStep,iStat) 
    ...
    IF (iStat .EQ. 0) THEN
        ...
        IF (ALine .NE. '') THEN
            CALL EstablishAbsolutePathFileName(TRIM(ALine),cWorkingDirectory,cAbsPathFileName)
            CALL InitElemWellPumpingOutFile(cAbsPathFileName,TimeStep%Unit,AppPumping%rPumpFactor,AppGrid%AppElement%ID,AppPumping%Wells%ID,AppPumping%ElemWellPumpingOutFile,iStat)
            IF (iStat .EQ. -1) RETURN
  ...
  ! -------------------------------------------------------------
  SUBROUTINE InitElemWellPumpingOutFile(cFileName,cUnitT,rPumpFactor,iElemIDs,iWellIDs,OutFile,iStat)
    ...
    IF (iGetFileType_FromName(cFileName) .NE. TXT) THEN
        CALL SetLastMessage('Element/well pumping output file must be a text file!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ...
    cTitleLines(1) = TRIM(cTitleLines(1)) // LineFeed // 'C                    AGRICULTURAL AND URBAN PUMPING AT ELEMENTS AND WELLS'
    cTitleLines(1) = TRIM(cTitleLines(1)) // LineFeed // 'C                         (UNIT=Same as in Input Pumping Rate File)'
    cTitleLines(1) = TRIM(cTitleLines(1)) // LineFeed // 'C' // REPEAT('-',100)
    WRITE (cTitleLines(1),'(A,I14,A)')   TRIM(cTitleLines(1))//LineFeed,iNCol      ,'      / NCOLPUMP' 
    WRITE (cTitleLines(1),'(A,F14.2,A)') TRIM(cTitleLines(1))//LineFeed,rPumpFactor,'      / FACTPUMP' 
    WRITE (cTitleLines(1),'(A,I14,A)')   TRIM(cTitleLines(1))//LineFeed,1          ,'      / NSPPUMP' 
    WRITE (cTitleLines(1),'(A,I14,A)')   TRIM(cTitleLines(1))//LineFeed,0          ,'      / NFQPUMP'
    WRITE (cTitleLines(1),'(A,14X,A)')   TRIM(cTitleLines(1))//LineFeed            ,'      / DSSFL' 
    cTitleLines(1) = TRIM(cTitleLines(1)) // LineFeed // 'C' // REPEAT('-',100)
...
! ******************************************************************
  ...
  SUBROUTINE Kill(AppPumping)
    ...
    !Close element pumping output files
