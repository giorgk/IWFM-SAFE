...
! ******************************************************************
  ...
  SUBROUTINE Well_New(cFileName,AppGrid,Stratigraphy,Wells,iStat)
    ...
    DO indxWell=1,NWell
        ...
        DO indxWell1=1,indxWell-1
            IF (iWellIDs(indxWell) .EQ. iWellIDs(indxWell1)) THEN
                CALL SetLastMessage('Well ID '//TRIM(IntToText(iWellIDs(indxWell)))//' is assigned to more than one well!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
        ...
        IF (Element .LT. 1) THEN
            CALL SetLastMessage('Well '// TRIM(IntToText(iWellIDs(indxWell))) // ' is outside the model domain!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxWell=1,NWell
        ...
        IF (iWell .EQ. 0) THEN
            CALL SetLastMessage('Well ID '//TRIM(IntToText(ID))//' listed for well pumping characteristics is not recognized!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (lProcessed(iWell)) THEN
            CALL SetLastMessage('Well ID '//TRIM(IntToText(ID))//' specified for well pumping characteristics is listed more than once!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (.NOT. ANY(Wells(iWell)%iDistMethod .EQ. f_iDistTypeArray)) THEN
            CALL SetLastMessage('Pumping distribution option (IOPTWL) for well ID ' // TRIM(IntToText(ID)) // ' is not recognized!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (.NOT. ANY(WellDest(iWell)%iDestType .EQ. f_iDestTypeArray)) THEN
            CALL SetLastMessage('Destination type for well ID '//TRIM(IntToText(ID))//' is not recognized!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        ELSE
            SELECT CASE (WellDest(iWell)%iDestType)
                ...
                CASE (f_iFlowDest_Element)
                    ...
                    IF (iElem .EQ. 0) THEN
                        CALL SetLastMessage('Destination element '//TRIM(IntToText(iElemID))//' listed for well '//TRIM(IntToText(ID))//' is not in the model!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
                    WellDest(iWell)%iDest       = iElem
                ...
                CASE (f_iFlowDest_Subregion)
                    ...
                    IF (iRegion.EQ. 0) THEN
                        CALL SetLastMessage('Destination subregion '//TRIM(IntToText(iRegionID))//' listed for well '//TRIM(IntToText(ID))//' is not in the model!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
    ...
    DO indxGroup=1,iNGroup
        ...
        DO indxGroup1=1,indxGroup-1
            IF (ID .EQ. ElemGroups(indxGroup1)%ID) THEN
                CALL SetLastMessage('Element group ID '//TRIM(IntToText(ID))//' for well pumping destinations is specified more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
        ...
        IF (ANY(Indices.EQ.0)) THEN
            CALL SetLastMessage('One or more elements listed in element group ID '//TRIM(IntToText(ID))//' listed for well pumping destination are not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxWell=1,NWell
       IF (WellDest(indxWell)%iDestType .EQ. f_iFlowDest_ElementSet) THEN
           ...
           IF (iDest .EQ. 0) THEN
               ...
               CALL SetLastMessage('Element group number '//TRIM(IntToText(iDestID))//' to which well ID '//TRIM(IntToText(ID))//' is delivered is not defined!',iFatal,ThisProcedure) 
               iStat = -1
               RETURN
           END IF
           ...
           IF (ElemGroups(iDest)%NElems .EQ. 0) THEN
               ID = Wells(indxWell)%ID
               CALL SetLastMessage('Element group '//TRIM(IntToText(iDest))//' as destination for well pumping '//TRIM(IntToText(ID))//' has no elements listed!',iFatal,ThisProcedure)
               iStat = -1
               RETURN
           END IF
