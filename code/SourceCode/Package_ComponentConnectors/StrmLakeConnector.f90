    ...
    ASSOCIATE (pStrmToLake   => Connector%StrmToLake   , &
        ...
        DO indx=1,Connector%NStrmToLake
            ...
            IF (iDest .EQ. 0) THEN
                ...
                CALL SetLastMessage('Lake '//TRIM(IntToText(iDestID))//' that receives flow from stream node '//TRIM(IntToText(iSourceID))//' is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF  
        ...
        DO indx=1,Connector%NBypassToLake
            ...
            IF (iDest .EQ. 0) THEN
                iSourceID = iStrmNodeIDs(iSource)
                CALL SetLastMessage('Lake '//TRIM(IntToText(iDestID))//' that receives flow from stream node '//TRIM(IntToText(iSourceID))//' by means of a bypass is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF    
        ...
        DO indx=1,Connector%NLakeToStrm
            ...
            IF (iDest .EQ. 0) THEN
                ...
                CALL SetLastMessage('Stream node '//TRIM(IntToText(iDestID))//' that receives flow from lake '//TRIM(IntToText(iSourceID))//' is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF    
