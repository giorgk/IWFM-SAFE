...
! ******************************************************************
  ...
  SUBROUTINE New(AppTileDrain,IsForInquiry,cFileName,cWorkingDirectory,NStrmNode,TimeStep,AppGrid,Stratigraphy,iStat) 
    ...
    CALL TileDrain_New(TDFile,AppGrid,Stratigraphy,NStrmNode,TimeUnitConductance_TD,AppTileDrain%TileDrains,iStat)
    IF (iStat .EQ. -1) RETURN
  ...
  ! --- NEW TileDrain DATA
  ! -------------------------------------------------------------
  SUBROUTINE TileDrain_New(InFile,AppGrid,Stratigraphy,NStrmNode,TimeUnitConductance,TileDrains,iStat)
    ...
    INTEGER                      :: NDrain,indx,indx1,iGWNodeID,iGWNode,NodeIDs(AppGrid%NNodes),ID,iNLayers
    ...
    DO indx=1,NDrain
        ...
        IF (iGWNode .EQ. 0) THEN
            CALL SetLastMessage('Groundwater node '//TRIM(IntTotext(iGWNodeID))//' listed for tile drain ID '//TRIM(IntToText(ID))//' is not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (TileDrains(indx)%iDestType.NE.f_iFlowDest_Outside .AND. TileDrains(indx)%iDestType.NE.f_iFlowDest_StrmNode) THEN
            CALL SetLastMessage('Flow destination type for tile drain '//TRIM(IntToText(ID))//' is not recognized!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (TileDrains(indx)%iDestType .EQ. f_iFlowDest_StrmNode) THEN
            IF (TileDrains(indx)%iDest.LT.1  .OR. TileDrains(indx)%iDest.GT.NStrmNode) THEN
                CALL SetLastMessage('Stream node number for tile drain '//TRIM(IntToText(ID))//' is not modeled!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END IF
        ...
        DO indx1=1,indx-1
            IF (TileDrains(indx)%ID .EQ. TileDrains(indx1)%ID) THEN
                CALL SetLastMessage('Tile drain ID number '//TRIM(IntToText(ID))//' is used more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
  ...
  SUBROUTINE SubIrig_New(InFile,AppGrid,Stratigraphy,TimeUnitConductance,SubIrigs,iStat)
    ...
    DO indx=1,NDrain
        ...
        CALL ConvertID_To_Index(iGWNodeID,NodeIDS,iGWNode)
        IF (iGWNode .EQ. 0) THEN
            CALL SetLastMessage('Groundwater node '//TRIM(IntTotext(iGWNodeID))//' listed for subsurface irrigation ID '//TRIM(IntToText(ID))//' is not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        DO indx1=1,indx-1
            IF (SubIrigs(indx)%ID .EQ. SubIrigs(indx1)%ID) THEN
                CALL SetLastMessage('Subsurface irrigation ID number '//TRIM(IntToText(ID))//' is used more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
...
! ******************************************************************
  ...
  SUBROUTINE GetDataList_AtLocationType(AppTileDrain,cDataList,cFileList,lBudgetType) 
    ...
    IF (AppTileDrain%TileDrainHyd%IsOutFileDefined()) THEN
        ...
        cDataList   = cDataList_AtTileDrain(f_iTileDrain)
        lBudgetType = .FALSE.
        CALL AppTileDrain%TileDrainHyd%GetOutFileName(cFileName)
        cFileList = ''
  ...
  SUBROUTINE GetModelData_AtLocation(AppTileDrain,iTDHydID,cDataType,cOutputBeginDateAndTime,cOutputEndDateAndTime,rFact_VL,nActualOutput,rOutputDates,rOutputValues,iStat)
    ...
    !Local variables
    INTEGER :: iTDIDs(AppTileDrain%NDrain)
    
    !Initialize
    iStat = 0
    
    !If data type is not recognized return
    IF (TRIM(cDataType) .NE. TRIM(cDataList_AtTileDrain(f_iTileDrain))) RETURN

    iTDIDs = AppTileDrain%TileDrains%ID
    CALL AppTileDrain%TileDrainHyd%ReadTDHyd_AtHydrographLocation(iTDIDs,f_iTileDrain,iTDHydID,cOutputBeginDateAndTime,cOutputEndDateAndTime,rFact_VL,nActualOutput,rOutputDates,rOutputValues,iStat)
