            ...
            DO indxNode=1,NNodes
                ...
                IF (iNData .GT. 1) THEN
                    ...
                    IF (MOD(iNData,2) .NE. 0) THEN
                        CALL SetLastMessage('Check that each groundwater node listed for stream node '//TRIM(IntToText(iStrmNodeID))//' is also given a rank for inundation order.',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
                    ...
                    DO indx=2,nGWNodes
                        ...
                        IF (iGWNodes(indx) .EQ. 0) THEN
                            CALL SetLastMessage('Groundwater node '//TRIM(IntToText(iGWNodeID))//' listed in stream reach '//TRIM(IntToText(pReach%ID))//' ('//TRIM(pReach%cName)//') for stream node '//TRIM(IntToText(iStrmNodeID))//' is not in the model!',iFatal,ThisProcedure)
                            iStat = -1
                            RETURN
                        END IF
                    ...
                    IF (nGWNodes .GT. 1) THEN
                        ...
                        IF (.NOT. ALL(lGoodNode(1:nGWNodes))) THEN
                            CALL SetLastMessage('Groundwater nodes listed for stream node '//TRIM(IntToText(iStrmNodeID))//' are not aligned along element faces!',iFatal,ThisProcedure)
                            iStat = -1
                            RETURN
                        END IF
                    END IF
