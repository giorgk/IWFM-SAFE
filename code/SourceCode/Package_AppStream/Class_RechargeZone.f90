...
! ******************************************************************
  ...
  SUBROUTINE RechargeZone_New(NDiver,iDiverIDs,iElemIDs,cDescription,InFile,RechargeZones,iStat)
    ...
    DO indxDiver=1,NDiver
        ...
        IF (iDiver .EQ. 0) THEN
            CALL SetLastMessage(TRIM(cDescription)//' ID '//TRIM(IntToText(ID))//' listed for recharge zones is not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (lProcessed(iDiver)) THEN
            CALL SetLastMessage(TRIM(cDescription)//' ID '//TRIM(IntToText(ID))//' is used more than once for recharge zone description!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (ErrorCode .NE. 0) THEN
            CALL SetLastMessage('Error allocating memory for recharge zones for '//TRIM(LowerCase(cDescription))//' '//TRIM(IntToText(ID))//'!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (NZones .GT. 0) THEN
            ...
            IF (ANY(pZone%Zones.EQ.0)) THEN
                CALL SetLastMessage('One or more elements listed as recharge zones for '//TRIM(LowerCase(cDescription))//' ID '//TRIM(IntToText(ID))//' are not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END IF
