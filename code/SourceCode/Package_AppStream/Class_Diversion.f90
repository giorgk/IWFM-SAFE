        ...
        ASSOCIATE (pDiver    => Diversions(indxDiver)      , &
            ...
            DO indxDiver1=1,indxDiver-1
                IF (ID .EQ. Diversions(indxDiver1)%Deli%ID) THEN 
                    CALL SetLastMessage('Diversion ID '//TRIM(IntToText(ID))//' is used more than once!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
            END DO
            ...
            IF (pDiver%iStrmNode .GT. 0) THEN
                ...
                IF (iStrmNode .EQ. 0) THEN
                    CALL SetLastMessage('Stream node '//TRIM(IntToText(pDiver%iStrmNode))//' where diversion '//TRIM(IntToText(ID))//' is originating from is not in the model!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
            ...
            SELECT CASE (pDeliDest%iDestType)
                ...
                CASE (f_iFlowDest_Element)
                    ...
                    IF (iElem .EQ. 0) THEN
                        CALL SetLastMessage('Element '//TRIM(IntToText(pDeliDest%iDest))//' that receives water from diversion '//TRIM(IntToText(ID))//' is not in the model!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
                ...
                CASE (f_iFlowDest_Subregion)
                    ...
                    IF (iRegion .EQ. 0) THEN
                        CALL SetLastMessage('Subregion '//TRIM(IntToText(pDeliDest%iDest))//' that receives water from diversion '//TRIM(IntToText(ID))//' is not in the model!',iFatal,ThisProcedure)
                        iStat = -1
                        RETURN
                    END IF
                ...
                CASE DEFAULT
                    CALL SetLastMessage('Destination type for diversion '//TRIM(IntToText(ID))//' is not recognized!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
            END SELECT
    ...
    DO indxGroup=1,iNGroup
        ...
        DO indxGroup1=1,indxGroup-1
            IF (ID .EQ. ElemGroups(indxGroup1)%ID) THEN
                CALL SetLastMessage('Element group ID '//TRIM(IntToText(ID))//' for diversion destinations is specified more than once!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        END DO
        ...
        IF (ANY(Indices.EQ.0)) THEN
            CALL SetLastMessage('One or more elements listed in element group ID '//TRIM(IntToText(ID))//' listed for diversion destinations are not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxDiver=1,NDiver
        IF (DeliDest(indxDiver)%iDestType .EQ. f_iFlowDest_ElementSet) THEN
            ...
            IF (iDest .EQ. 0) THEN
                ...
                CALL SetLastMessage('Element group number '//TRIM(IntToText(iDestID))//' to which diversion '//TRIM(IntToText(ID))//' is delivered is not defined!',iFatal,ThisProcedure)  
                iStat = -1
                RETURN
            END IF
            ...
            IF (ElemGroups(iDest)%NElems .EQ. 0) THEN
                ID = Diversions(indxDiver)%Deli%ID
                CALL SetLastMessage('Element group '//TRIM(IntToText(iDestID))//' as destination for diversion '//TRIM(IntToText(ID))//' has no elements listed!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
