  ...
  SUBROUTINE HydrographList_New(AppGrid,Stratigraphy,iGWNodeIDs,iHydFor,NHyd,FactXY,InFile,Hyd_AtNode,Hyd_AtXY,OrderedHydList,iStat)
    ...
    DO indx=1,NHyd
        ...
        DO indx1=1,3
            ...
            IF (iLoc .EQ. 0) THEN
                CALL SetLastMessage('Error in data entry for '//TRIM(cHydDescriptor)//' hydrograph specification '//TRIM(IntToText(ID))//'!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        ...
        IF (iHydType .EQ. f_iHyd_AtXY) THEN
            DO indx1=1,2
                ...
                iLoc = FirstLocation(' ',ALine)
                IF (iLoc .EQ. 0) THEN
                    CALL SetLastMessage('Error in data entry for '//TRIM(cHydDescriptor)//' hydrograph specification ID '//TRIM(IntToText(ID))//'!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
                ALine = ADJUSTL(ALine(iLoc:LEN_TRIM(ALine)))
        ...
        ELSE
            ...
            IF (iNodeIndex .EQ. 0) THEN
                CALL SetLastMessage('Node number listed for '//TRIM(cHydDescriptor)//' hydrograph specification ID '//TRIM(IntToText(ID))//' is not in the model!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
            iLoc = FirstLocation(' ',ALine)
...
! ******************************************************************
  ...
  SUBROUTINE ReadHydrograph_AtLocation(HydOutput,iHydID,cOutputBeginDateAndTime,cOutputEndDateAndTime,rOutputFactor,rConversionFactor,nActualOutput,rOutputDates,rOutputValues,iStat)
    ...
    IF (iHydIndex .EQ. 0) THEN
        CALL SetLastMessage('Groundwater hydrograph ID '//TRIM(IntToText(iHydID))//' is not in the model!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
