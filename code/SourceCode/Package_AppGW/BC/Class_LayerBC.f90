...
! ******************************************************************
  ...
  SUBROUTINE LayerBC_InitSpecifiedFlowBC(cFileName,NNodes,NodeIDs,Stratigraphy,cTimeUnit,LayerBC,iStat)
    ...
    DO indx=1,NQB
        ...
        IF (iNode .EQ. 0) THEN
            CALL SetLastMessage('Node number '//TRIM(IntToText(ID))//' listed in groundwater specified flow boundary condition file is not in the model.',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (.NOT. Stratigraphy%ActiveNode(iNode,iLayer)) THEN
            CALL SetLastMessage('Specified flow boundary node '//TRIM(IntToText(ID))//' in layer '//TRIM(IntToText(iLayer))//' is an inactive node!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
  ...
  SUBROUTINE LayerBC_InitSpecifiedHeadBC(cFileName,NNodes,NodeIDs,Stratigraphy,GWHeads,LayerBC,iStat)
    ...
    DO indx=1,NHB
        ...
        !Make sure iNode is in model range
        IF (iNode .EQ. 0) THEN
            CALL SetLastMessage('Node number '//TRIM(IntToText(ID))//' listed in groundwater specified head boundary condition file is not in the model.',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        !Make sure boundary node is an active node
        IF (.NOT. Stratigraphy%ActiveNode(iNode,iLayer)) THEN
            CALL SetLastMessage('Specified head boundary node '//TRIM(IntToText(ID))//' in layer '//TRIM(IntToText(iLayer))//' is an inactive node!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
  ...
  SUBROUTINE LayerBC_InitGeneralHeadBC(cFileName,NNodes,NodeIDs,Stratigraphy,cTimeUnit,LayerBC,iStat)
    ...
    DO indx=1,NGB
        ...
        !Make sure iNode is in model range
        IF (iNode .EQ. 0) THEN
            CALL SetLastMessage('Node number '//TRIM(IntToText(ID))//' listed in groundwater general head boundary condition file is not in the model.',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        !Make sure boundary node is an active node
        IF (.NOT. Stratigraphy%ActiveNode(iNode,iLayer)) THEN
            CALL SetLastMessage('General head boundary node '//TRIM(IntToText(ID))//' in layer '//TRIM(IntToText(iLayer))//' is an inactive node!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
  ...
  SUBROUTINE LayerBC_InitConstrainedGeneralHeadBC(cFileName,NNodes,NodeIDs,Stratigraphy,cTimeUnit,LayerBC,iStat)
    ...
    DO indx=1,NCGB
        ...
        !Make sure iNode is in model range
        IF (iNode .EQ. 0) THEN
            CALL SetLastMessage('Node number '//TRIM(IntToText(ID))//' listed in groundwater constrained general head boundary condition file is not in the model.',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        !Make sure boundary node is an active node
        IF (.NOT. Stratigraphy%ActiveNode(iNode,iLayer)) THEN
            CALL SetLastMessage('Constrained general head boundary node '//TRIM(IntToText(ID))//' in layer '//TRIM(IntToText(iLayer))//' is an inactive node!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    ...
    DO indxLayer=1,SIZE(LayerBC)
        ...
        ASSOCIATE (pLayerBC => LayerBC(indxLayer))
            ...
            DO indxBC=1,pLayerBC%NSpecFlowBC
                ...
                CALL Matrix%UpdateRHS(f_iGWComp,iGWNode,rUpdateValues)
            END DO
