  ...
  SUBROUTINE BCFlowOutput_New(IsForInquiry,InFile,cWorkingDirectory,NodeIDs,NLayers,LayerBC,UNITVLOU,TimeStep,BCFlowOutput,iStat)
    ...
    DO indx=1,NOUTB
        ...
        DO indx1=1,3
            ...
            IF (iLoc .EQ. 0) THEN
                CALL SetLastMessage('Error in data entry for boundary node flow hydrograph specification '//TRIM(IntToText(indx))//'!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        ...
        IF (iHydNode .EQ. 0) THEN
            CALL SetLastMessage('Node '//TRIM(IntToText(IDNode))//' listed for boundary node hydrograph printing is not in the model!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
